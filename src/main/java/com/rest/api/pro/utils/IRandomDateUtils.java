...
public interface IRandomDateUtils {
    static IRandomDateUtils pastSixMonthsDate() {
        ...
        return (dateTimeFormatter) -> startDate.get().datesUntil(endDate.get()).map(localDate -> localDate.format(dateTimeFormatter)).collect(Collectors.toList()).stream().findAny().orElseThrow();
    }
